...
namespace GameTitleRentalTest
{
	TEST_CLASS(GameTitleRentalTest)
	{
	public:
		...
		TEST_METHOD(TestMethod_인기일반타이틀)
		{
			...
			Customer customer("pororo");

			// rental01 - title 1, popular 2, daysRented 3
			int priceCode = 2;
			GameTitle gameTitle01("GTA5", priceCode);
			int daysRented = 3;
			...
			priceCode = 0;
			GameTitle gameTitle02("MonsterHunter: RISE", priceCode);
			daysRented = 5;
		...
		TEST_METHOD(TestMethod2_최신인기타이틀)
		{
			...
			priceCode = 2;
			GameTitle gameTitle02("COOKIERUN:KINGDOM", priceCode);
			daysRented = 2;
			Rental rental02(gameTitle02, daysRented);
			customer.addRental(rental02);
